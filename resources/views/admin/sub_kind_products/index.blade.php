...
<br><br>
{{--      @if($user->role_id == 1)--}}
          <a href="{{ route('admin_sub_kind_products.create') }}">Створити підвид продукту</a>
{{--      @endif--}}
<div>
    <ul>
        @if(isset($KindProduct))
            <h1><a href="{{route('admin_kind_products.index')}}">{{$KindProduct->name}}</a></h1>
        ...
        @else
            @foreach($kind_products as $kind_product)
                <h1><a href="{{route('admin_kind_products.index')}}">{{$kind_product->name}}</a></h1>
                @foreach($sub_kind_products as $sub_kind_product)
