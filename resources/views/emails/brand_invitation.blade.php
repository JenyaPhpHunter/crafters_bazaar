...
<div style="max-width: 600px; margin: 0 auto; background: #fff; padding: 30px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
    ...
    <p style="text-align: center; margin: 30px 0;">
        <a href="{{ config('app.url') }}/brands/{{ $brand->id }}"
           style="display: inline-block; padding: 12px 24px; background-color: #0d6efd; color: #fff; text-decoration: none; border-radius: 5px;">
            Переглянути бренд
        </a>
    </p>
...
{{--<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;">--}}
{{--<div style="max-width: 600px; margin: 0 auto; background: #fff; padding: 30px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">--}}
{{--    <h2 style="color: #0d6efd;">Запрошення до бренду "{{ $brand->title }}"</h2>--}}
{{--    @if($brand->image_path)--}}
{{--        <div style="text-align: center; margin: 20px 0;">--}}
{{--            <img src="{{ config('app.url') . '/storage/' . $brand->image_path }}"--}}
{{--                 alt="{{ $brand->title }}"--}}
