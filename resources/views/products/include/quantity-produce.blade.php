@php
    ...
    $termCreation = $product?->term_creation ?? 0;
    $canProduce   = $termCreation > 0;
@endphp
...
<div class="quantity-produce-container">
    ...
    @if($isView)
        ...
        <div class="view-field mb-3">
            <strong>Кількість у наявності:</strong>
            <span class="{{ $stockBalance > 0 ? 'text-success' : 'text-danger' }}">
                {{ $stockBalance }} шт.
            </span>
        </div>
        ...
        @if($canProduce)
            <div class="view-field">
                <strong>Термін виготовлення:</strong>
                {{ $termCreation }} днів
            </div>
        @endif

    @else
        <!-- Режим РЕДАГУВАННЯ (оригінальний код) -->
        <div class="quantity-field">
            ...
            <div class="input-wrapper">
                ...
                <input type="number" id="stock_balance" name="stock_balance"
                       ...
                       value="{{ old('stock_balance', $stockBalance) }}" min="0" step="1">
        ...
        <div class="checkbox-field">
            <div class="checkbox-wrapper">
                <input type="checkbox" id="can_produce" name="can_produce" {{ $canProduce ? 'checked' : '' }}>
                <label for="can_produce" class="checkbox-label">Можу виготовити ще</label>
            </div>
        </div>

        <div class="term-field" id="term_creation_container" style="{{ $canProduce ? '' : 'display: none;' }}">
            <label for="term_creation" class="form-label">Термін виготовлення</label>
            <div class="input-wrapper">
                <button type="button" class="qty-btn minus" aria-label="Зменшити">-</button>
                <input type="number" id="term_creation" name="term_creation"
                       class="form-control qty-input"
                       value="{{ old('term_creation', $termCreation) }}" min="0" step="1">
                <button type="button" class="qty-btn plus" aria-label="Збільшити">+</button>
